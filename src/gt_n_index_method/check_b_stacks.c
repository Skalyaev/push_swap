...
int b_stack_unascending(t_data *data, t_stack *backup, t_stack *next)
{
        ...
        while (data->b->nb != backup->nb)
        {
                ...
                if (!next)
                        next = data->b_start;
                if (data->b->nb < next->nb && next->nb != data->b_max && ++data->b_unascending)
                        break;
                ++data->to_raw;
                data->b = data->b->next;
                if (!data->b)
        ...
        if (data->to_raw > 0 && ++data->b_r_rot_needed && data->b_unascending)
                return (0);
        if ((data->to_raw < 0 || (data->to_raw == 0 && data->b_unascending)) && ++data->b_rot_needed)
                return (0);
        return (1);
}
...
int check_b_stack_ter(t_stack *a, t_stack *backup, t_data *data)
{
        ...
        if (data->b->index == a->index + offset && data->a_ready && ++data->b_push_needed)
                return (0);
        if (data->b->nb < next->nb && (next->nb != data->b_max || data->b->nb != data->b_min) && ++data->b_swap_needed)
                return (0);
        backup = data->b;
